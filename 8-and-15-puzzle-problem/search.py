...
class EightPuzzle(Problem):
    ...
    def h(self, node):
        ...
        for i in range(len(node.state)):
            position_difference = abs(self.goal.index(i) - node.state.index(i))
            if i is not 0:
                kalan = position_difference % 3
                bolum = position_difference / 3
                distance += kalan + int(math.floor(bolum))
                if abs(self.goal.index(i) % 3 - node.state.index(i) % 3) == 2 and position_difference % 3 == 1:
                    distance += 2
                # print("i: " + str(i) + " goal-index: " + str(node.state.index(i)) + " current-index: " + str(node.state.index(i)) + " bolum: " + str(bolum) + ": kalan: " + str(kalan) + ": distance: " + str(distance))
        return distance
